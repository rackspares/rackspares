# RackSpares

**RackSpares - Open source data center warehouse management**

RackSpares is a self-hosted inventory management tool built for data center and IT infrastructure teams. Track your spare servers, networking gear, cables, and other hardware with a clean, fast web interface.

---

## Features

- **Inventory CRUD** — Add, edit, and delete items with fields for name, category, quantity, location, status, and description
- **Search & Filter** — Real-time search across name, description, and location; filter by category, status, and stale verification age
- **Status Tracking** — Mark items as Available, In Use, Faulty, or Retired
- **Stale Date Tracking** — "Verify Stock" button timestamps physical checks; amber warning at 30+ days, red at 90+ days
- **Dashboard Stats** — At-a-glance totals for inventory counts, quantities, fault status, and stale items
- **Role-Based Access Control** — Three roles: Admin (full access), Manager (inventory CRUD), Viewer (read-only)
- **User Management** — Admins can create users, assign roles, reset passwords, and enable/disable accounts
- **Audit Log** — Every inventory change is logged with user, action, and a JSON diff; filterable by user, action, and date range
- **Password Change** — Any user can change their own password from within the app
- **Authentication** — JWT-based login with a seeded admin account; disabled accounts are rejected at login
- **Fully Containerized** — Postgres + FastAPI + React, all wired up with Docker Compose

---

## Quick Start

**Prerequisites:** Docker and Docker Compose

```bash
git clone https://github.com/yourorg/rackspares.git
cd rackspares
docker compose up
```

Then open [http://localhost:3000](http://localhost:3000) and log in:

| Field    | Default |
|----------|---------|
| Username | `admin` |
| Password | `admin` |

> **Note:** Change the default credentials and `SECRET_KEY` in `docker-compose.yml` before exposing to a network.

### Environment Variables

| Variable         | Default                                              | Description                |
|------------------|------------------------------------------------------|----------------------------|
| `DATABASE_URL`   | `postgresql://rackspares:rackspares@db/rackspares`   | Postgres connection string |
| `SECRET_KEY`     | `changeme-in-production`                             | JWT signing secret         |
| `ADMIN_USERNAME` | `admin`                                              | Seeded admin username      |
| `ADMIN_PASSWORD` | `admin`                                              | Seeded admin password      |

---

## Optional Services

RackSpares can connect to three optional services. Configure them via **Admin → Settings → External Services**.

| Service | Purpose |
|---------|---------|
| [NetBox](https://netbox.dev) | Network documentation & DCIM — rack layouts, device inventory |
| [Paperless-ngx](https://docs.paperless-ngx.com) | Document management — receipts, warranties, datasheets |
| [n8n](https://n8n.io) | Workflow automation — reorder alerts, Slack/email notifications |

Enter the URL and API key for each service, then click **Save & Test**. RackSpares will verify the connection and show live status on the Services page.

### Deploying a service alongside RackSpares

The `services/` directory contains bundled Docker Compose files for each optional service. To deploy one manually:

```bash
# NetBox
docker compose -f services/netbox/docker-compose.yml up -d

# Paperless-ngx
docker compose -f services/paperless/docker-compose.yml up -d

# n8n
docker compose -f services/n8n/docker-compose.yml up -d
```

Then connect the running instance in RackSpares via Admin → Settings → External Services.

---

## Project Structure

```
rackspares/
├── backend/          # FastAPI application
│   ├── main.py       # App entry point, startup seeding
│   ├── models.py     # SQLAlchemy ORM models
│   ├── schemas.py    # Pydantic request/response schemas
│   ├── database.py   # DB session and engine
│   └── routers/      # Auth, inventory, and audit route handlers
├── frontend/         # React application
│   └── src/
│       ├── pages/    # Login, Inventory, UserManagement, AuditLog, ChangePassword
│       └── components/ # Navbar, form, status badge
├── docker-compose.yml
└── VERSION
```

---

## Roadmap

Planned features for future releases:

- **BOM Management** — Build bills of materials with a shopping cart workflow for procurement
- **Barcode Scanning** — Scan barcodes and QR codes via browser camera for fast check-in/out
- **NetBox Integration** — Sync devices and racks from a NetBox instance
- **Consumable Inventory** — Track consumables (e.g. thermal paste, screws) with low-stock reorder alerts
- **Serial Number Tracking** — Per-unit serial tracking with full history
- **Paperless-ngx Integration** — Link datasheets and invoices from a Paperless-ngx instance
- **N8n / Node-RED Integration** — Trigger automation workflows on inventory events
- **Local LLM Support** — Natural language search and anomaly detection via a local LLM (Ollama)

---

## License

Apache 2.0 — see [LICENSE](LICENSE)
